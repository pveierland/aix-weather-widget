...
public class AixWidget extends AppWidgetProvider {
	...
	private static Bitmap drawWidget(Context context, int widgetWidth, int widgetHeight, final float dp, Uri viewUri, String locationName, ArrayList<Temperature> temperatureValues, ArrayList<Rain> rainValues, float[] rainDataValues, float[] rainDataMinValues, float[] rainDataMaxValues) {
		...
		for (Temperature t : temperatureValues) {
			...
			if (t.time >= time && t.time <= time + 24 * DateUtils.HOUR_IN_MILLIS)
			{
		...
		Path path = buildPath(temperatureValues, tempRangeMax, tempRangeMin, time, time + 24 * DateUtils.HOUR_IN_MILLIS);
		...
		for (int i = 0; i < rainDataValues.length; i++) {
			...
			RectF rainRect = new RectF(
					...
					graphRect.left + Math.round((i + 1) * cellSizeX),
					graphRect.bottom);
		...
		int iconHourResolution = Integer.MAX_VALUE;
		for (Rain rain : rainValues) {
			int res = (int)((rain.timeTo - rain.timeFrom) / DateUtils.HOUR_IN_MILLIS);
			if (res < iconHourResolution && res >= numCellsPerIcon) {
				iconHourResolution = res;
			}
		}
		
		long rtp = time + iconHourResolution * DateUtils.HOUR_IN_MILLIS / 2;
		
		Rect dest = new Rect();
		for (Rain rain : rainValues) {
			int res = (int)((rain.timeTo - rain.timeFrom) / DateUtils.HOUR_IN_MILLIS);
			if (res == iconHourResolution && rain.timeFrom >= time && rain.timeTo <= time + 24 * DateUtils.HOUR_IN_MILLIS) {
				...
				for (int i = 1; i < temperatureValues.size(); i++) {
					...
					if (!Float.isNaN(val)) {
						int icon = rain.weatherIcon - 1;
						if (icon >= 0 && icon < iconIds.length) { // Ensure that access is within array
							dest.left = (int)Math.round(graphRect.left + Math.round(((float)(iconTimePos - time) / (float)DateUtils.HOUR_IN_MILLIS) * cellSizeX) - iconWidth / 2.0f);
							...
							Calendar lazyCalendar = Calendar.getInstance(TimeZone.getTimeZone("UTC"));
							lazyCalendar.setTimeInMillis(iconTimePos);
							lazyCalendar.set(Calendar.HOUR_OF_DAY, 0);
							lazyCalendar.set(Calendar.MINUTE, 0);
							lazyCalendar.set(Calendar.SECOND, 0);
							lazyCalendar.set(Calendar.MILLISECOND, 0);
							...
							Cursor sunCursor = context.getContentResolver().query(
									...
									null,
									AixForecastsColumns.TIME_FROM + "=" + lazyCalendar.getTimeInMillis() + " AND " +
									AixForecastsColumns.SUN_RISE + "<" + iconTimePos + " AND " +
									AixForecastsColumns.SUN_SET + ">" + iconTimePos,
									null,
									null);
							if (sunCursor != null) {
								if (sunCursor.getCount() == 0) {
									isDay = false;
								}
								sunCursor.close();
							}
							...
							if (isDay) {
								weatherIcon = ((BitmapDrawable)context.getResources().getDrawable(iconIds[rain.weatherIcon])).getBitmap();
							} else {
								weatherIcon = ((BitmapDrawable)context.getResources().getDrawable(iconIds_night[rain.weatherIcon])).getBitmap();
							}
							if (weatherIcon != null) {
								canvas.drawBitmap(weatherIcon, null, dest, null);
							}
							
							rtp = iconTimePos + iconHourResolution * DateUtils.HOUR_IN_MILLIS;
							break;
						}
					}
				}
		...
		// Draw time stamp labels and horizontal notchet
		...
		for (int i = 2; i < 24; i+= 2) {
			...
			String hourLabel = String.format("%02d", (startHour + i) % 24);
...
//					textBottom + locationNameRect.bottom);
	...
	public static Bitmap buildView(Context context, Uri viewUri, boolean landscape) {
		...
		Cursor cursor = resolver.query(
				...
				new String[] { Long.toString(time - 12 * DateUtils.HOUR_IN_MILLIS), Long.toString(time + 36 * DateUtils.HOUR_IN_MILLIS) },
		...
		float[] rainDataMinValues = new float[24];
		float[] rainDataValues = new float[24];
		float[] rainDataMaxValues = new float[24];
		...
		cursor = resolver.query(
				...
				new String[] { Long.toString(time), Long.toString(time + 24 * DateUtils.HOUR_IN_MILLIS) },
				AixForecastsColumns.TIME_FROM + " ASC");
		...
		for (Rain r : rainValues) {
			int startIndex = (int)((r.timeFrom - time) / DateUtils.HOUR_IN_MILLIS);
			int stopIndex = (int)((r.timeTo - time) / DateUtils.HOUR_IN_MILLIS);
			
			startIndex = Math.max(0, startIndex);
			stopIndex = Math.min(24, stopIndex);
		...
		return drawWidget(context, (int)widgetWidth, (int)widgetHeight, dp, viewUri, locationName, temperatureValues, rainValues,
